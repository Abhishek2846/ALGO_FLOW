...
import { useHeapVisualizer } from '../../hooks/useHeapVisualizer';
...
export default function HeapPage() {
  const meta = getById('heap')!;
  const {
    heap, highlightIdx, treeNodes, treeRootId,
    isPlaying, isDone, activeLine, description, speed, lastExtracted,
    setSpeed, play, pause, step, insert, extractMax, clear, resetSample,
  } = useHeapVisualizer();
  ...
  const doInsert = () => {
    ...
    if (heap.length >= 30) { setOpError('Max 30 elements'); return; }
    setOpError('');
    insert(v);
    setInsertVal('');
  };
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', color: '#94a3b8' }}>
                  {isDone ? 'Heap Active' : isPlaying ? 'Heapifying...' : 'Visualizer Ready'}
                </span>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div className="glass-panel" style={{ borderRadius: '0.75rem', overflow: 'hidden', border: `1px solid ${ACCENT}33`, boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
              ...
              <div style={{ padding: '0.75rem 1rem', background: '#0f111a', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {lastExtracted !== null && (
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', color: '#4fd1a5', background: 'rgba(79,209,165,0.1)', padding: '0.2rem 0.6rem', borderRadius: '1rem' }}>
                    Extracted Max: {lastExtracted}
                  </div>
                )}
              </div>
              ...
              <div style={{ padding: '0.6rem 1rem', background: '#13151f', borderBottom: '1px solid rgba(255,255,255,0.05)', fontFamily: 'var(--font-mono)', fontSize: '0.8rem', color: '#94a3b8', minHeight: 32 }}>
                {description || 'Max-Heap visualization ready.'}
              </div>
              ...
              <div style={{ padding: '1.5rem', background: 'radial-gradient(circle at center, rgba(15,17,26,1) 0%, rgba(8,9,14,1) 100%)', position: 'relative' }}>
                ...
                <TreeCanvas nodes={treeNodes} rootId={treeRootId} />
              </div>

              {/* Underlying Array View */}
              <div style={{ padding: '1rem', background: '#13151f', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
                  <span className="material-symbols-outlined" style={{ fontSize: '1rem', color: ACCENT }}>table_rows</span>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', color: '#e2e8f0', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Underlying Array</span>
                </div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {heap.length === 0 ? (
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', color: '#64748b' }}>[ empty ]</span>
                  ) : heap.map((val, i) => {
                    const isHighlit = highlightIdx.includes(i);
                    return (
                      <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                        <div style={{
                          width: 40, height: 40,
                          background: isHighlit ? 'rgba(242,184,75,0.25)' : 'rgba(18,19,26,0.8)',
                          border: `1.5px solid ${isHighlit ? ACCENT : 'rgba(255,255,255,0.1)'}`,
                          borderRadius: '0.4rem',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          fontFamily: 'var(--font-mono)', fontSize: 13, fontWeight: 700,
                          color: isHighlit ? ACCENT : '#fff',
                          boxShadow: isHighlit ? `0 0 10px ${ACCENT}66` : 'none',
                          transition: 'all 0.2s ease',
                        }}>
                          {val}
                        </div>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#64748b' }}>{i}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div style={{ padding: '1rem', background: '#0f111a', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <ControlBtn id="ctrl-clear" icon="delete_sweep" label="Clear Heap" onClick={clear} disabled={false} />
                  </div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'center', paddingTop: '0.5rem', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <input id="ctrl-heap-insert-val" type="number" placeholder="Val" value={insertVal}
                      onChange={(e) => { setInsertVal(e.target.value); setOpError(''); }}
                      onKeyDown={(e) => e.key === 'Enter' && doInsert()}
                      style={{ width: 80, padding: '0.35rem 0.5rem', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '0.4rem', color: '#fff', fontFamily: 'var(--font-mono)', fontSize: '0.8rem' }} />
                    <button id="ctrl-heap-insert" onClick={doInsert} style={{ padding: '0.35rem 0.75rem', background: `${ACCENT}26`, border: `1px solid ${ACCENT}66`, borderRadius: '0.4rem', color: ACCENT, fontFamily: 'var(--font-mono)', fontSize: '0.75rem', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                      + Insert
                    </button>
                  </div>
                  <button id="ctrl-heap-extract" onClick={extractMax} disabled={heap.length === 0}
                    style={{ padding: '0.35rem 0.75rem', background: 'rgba(239,100,97,0.15)', border: '1px solid rgba(239,100,97,0.3)', borderRadius: '0.4rem', color: '#ef6461', fontFamily: 'var(--font-mono)', fontSize: '0.75rem', cursor: heap.length === 0 ? 'not-allowed' : 'pointer', opacity: heap.length === 0 ? 0.4 : 1, whiteSpace: 'nowrap' }}>
                    ↑ Extract Max
                  </button>
                </div>
            ...
            <div className="glass-panel" style={{ borderRadius: '0.75rem', padding: '1.5rem', background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              <div style={{ display: 'grid', gap: '0.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem', background: 'rgba(255,255,255,0.02)', borderRadius: '0.5rem', border: '1px solid rgba(255,255,255,0.05)', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.06)'} onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'}>
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', color: '#f2b84b', fontSize: '0.9rem' }}>O(log n)</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem', background: 'rgba(255,255,255,0.02)', borderRadius: '0.5rem', border: '1px solid rgba(255,255,255,0.05)', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.06)'} onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'}>
                  <span style={{ color: '#94a3b8', fontSize: '0.9rem' }}>Extract Max</span>
                  <span style={{ fontFamily: 'var(--font-mono)', color: '#f2b84b', fontSize: '0.9rem' }}>O(log n)</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem', background: 'rgba(255,255,255,0.02)', borderRadius: '0.5rem', border: '1px solid rgba(255,255,255,0.05)', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.06)'} onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'}>
