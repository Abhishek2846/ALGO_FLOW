...
import SearchCanvas from '../../components/ui/SearchCanvas';
import DifficultyBadge from '../../components/ui/DifficultyBadge';
import CodeEditorPanel from '../../components/ui/CodeEditorPanel';
import Footer from '../../components/layout/Footer';
import type { AlgorithmMeta, SearchStep } from '../../types';
...
const PSEUDOCODES: Record<string, string[]> = {
  ...
  'interpolation-search': [
    ...
    '  lo ← 0, hi ← n - 1',
    '  while lo ≤ hi and target ∈ [A[lo], A[hi]]:',
    '    comparisons++',
  ...
  return (
    <button
      ...
      style={{
        ...
        background: accent ? 'rgba(110, 107, 244, 0.2)' : 'rgba(18, 19, 26, 0.8)',
...
export default function SearchPageTemplate({
  ...
  const {
    displayBars, currentStep, target, isPlaying, isDone, speed, arraySize,
    setSpeed, setArraySize, changeTarget,
    play, pause, step, reset, setCustomArray,
  } = useSearchVisualizer(generateSteps, requiresSorted);

  const [customInput, setCustomInput] = useState('');
  const [targetInput, setTargetInput] = useState(String(target));
  const [inputError, setInputError] = useState('');

  const pseudocode = PSEUDOCODES[meta.id] ?? meta.pseudocode ?? [];
  ...
  const applyCustom = () => {
    ...
    if (parts.some(isNaN) || parts.length < 2) {
      setInputError('Enter ≥ 2 comma-separated integers.'); return;
    }
    setInputError('');
    setCustomArray(parts);
  };

  const applyTarget = () => {
    const t = parseInt(targetInput, 10);
    if (isNaN(t)) { setInputError('Target must be a number.'); return; }
    setInputError('');
    changeTarget(t);
  };
      ...
      <div style={{ padding: '2rem var(--gutter)', maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div style={{ 
          ...
          <div style={{ flex: '1 1 280px', minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', color: '#94a3b8' }}>
                  {isDone ? 'Search Completed' : isPlaying ? 'Searching...' : 'Visualizer Ready'}
                </span>
              </div>
            </div>
            ...
            {requiresSorted && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginTop: '0.75rem', padding: '4px 12px', background: 'rgba(79,209,165,0.1)', border: '1px solid rgba(79,209,165,0.3)', borderRadius: '0.5rem', fontFamily: 'var(--font-mono)', fontSize: '0.8rem', color: '#4fd1a5' }}>
                <span className="material-symbols-outlined" style={{ fontSize: '1rem' }}>info</span>
                Sorted Array Required (Auto-Sorted)
              </span>
            )}
          </div>
            ...
            <div className="glass-panel" style={{ borderRadius: '0.75rem', overflow: 'hidden', border: `1px solid ${ACCENT}33`, boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
              ...
              <div style={{ padding: '0.75rem 1rem', background: '#0f111a', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span className="material-symbols-outlined" style={{ fontSize: '1rem', color: ACCENT }}>manage_search</span>
                  {meta.id}.viz
                </div>
              ...
              <div style={{ padding: '1rem', background: '#0f111a', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center', paddingTop: '0.5rem', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', color: '#94a3b8' }}>Target:</span>
                    <input
                      id="ctrl-target"
                      type="number"
                      value={targetInput}
                    ...
                    />
                    <button id="ctrl-set-target" onClick={applyTarget} style={{ padding: '0.35rem 0.75rem', background: `${ACCENT}26`, border: `1px solid ${ACCENT}66`, borderRadius: '0.4rem', color: ACCENT, fontFamily: 'var(--font-mono)', fontSize: '0.75rem', cursor: 'pointer' }}>
                      Set
                    </button>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    ...
                    <input id="ctrl-size" type="range" min={6} max={32} step={1} value={arraySize}
                      onChange={(e) => setArraySize(parseInt(e.target.value, 10))}
                      style={{ width: 80, accentColor: ACCENT }} />
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', color: ACCENT }}>{arraySize}</span>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flex: 1, minWidth: 200 }}>
                    ...
                    />
                    <button id="ctrl-set-array" onClick={applyCustom} style={{ padding: '0.35rem 0.75rem', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '0.4rem', color: '#94a3b8', fontFamily: 'var(--font-mono)', fontSize: '0.75rem', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                      Apply
                    </button>
                  </div>
                </div>
                {inputError && <p style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#ef6461', margin: 0 }}>{inputError}</p>}
              </div>
